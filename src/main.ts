...
import { debugLog, path, ConvertImage } from "./utils";
...
export default class PastePngToJpegPlugin extends Plugin {
	...
	async renameFile(file: TFile) {
		...
		if (this.settings.pngToJpeg) {
			...
			let arrayBuffer: ArrayBuffer = await ConvertImage(
				imgBlob,
	...
	async generateNewName(file: TFile, activeFile: TFile): Promise<string> {
		...
		const extension = this.settings.pngToJpeg ? "webp" : file.extension;
...
class SettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		new Setting(containerEl)
			.setName("Png to Webp")
			.setDesc(
				`Paste images from ClipBoard to notes by copying them through various screenshot software,turn on this feature will automatically convert png to Webp, and more quality compression volume.`
			)
			.addToggle((toggle) =>
				toggle
		...
		new Setting(containerEl)
			.setName("Auto Rename")
			.setDesc(
				`Automatically names the image with the name of the previous note +'-'+ the current timestamp + '.' + file type, for example, the image in test.md will be named test-1652261724173.webp`
			)
			.addToggle((toggle) =>
				toggle
		...
		new Setting(containerEl)
			...
			.setDesc(
				`Automatically move images to the image directory,If you do not set the default directory for attachments, then it will be stored directly under the image/ folder in the same directory as the notes, if there is a default directory, then it will be stored under the image/ folder in the Magician directory`
			)
			.addToggle((toggle) =>
				toggle
